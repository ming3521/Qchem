...
def select_largest_gradient(gradients: Sequence[float], pool: Sequence[AdaptOperator], threshold: float = 1e-4):
    """Select the operator with the largest absolute ADAPT gradient.

    ``gradients`` may be a Python list, tuple, NumPy array, or Torch tensor.
    NumPy arrays cannot be tested with ``if not gradients`` when they contain
    more than one element, because their truth value is ambiguous.  We therefore
    normalize the input to a one-dimensional NumPy array and check ``size``
    explicitly.
    """

    if isinstance(gradients, torch.Tensor):
        grad_array = gradients.detach().cpu().numpy()
    else:
        grad_array = np.asarray(gradients, dtype=float)

    grad_array = np.ravel(grad_array).astype(float)

    if grad_array.size != len(pool):
        raise ValueError(
            f"Gradient list and operator pool must have the same length: "
            f"got {grad_array.size} gradients and {len(pool)} operators"
        )
    if grad_array.size == 0:
        return None, 0.0, True

    idx = int(np.argmax(np.abs(grad_array)))
    max_grad = float(grad_array[idx])
    converged = abs(max_grad) < float(threshold)
